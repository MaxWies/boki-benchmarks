...
export REGISTRATION_EVALUATION=false
...
if [[ $WORKLOAD == empty-tag ]]; then
    ...
    export SHARED_TAGS_CAPACITY=20
elif [[ $WORKLOAD == mix-read-heavy ]]; then
    export APPEND_TIMES=1
    ...
    export TAG_READ_PERCENTAGES=0,20,20,20,20,20
    export SHARED_TAGS_CAPACITY=20
elif [[ $WORKLOAD == mix-low-concurrency ]]; then
    export APPEND_TIMES=1
    export READ_TIMES=1
    export OPERATION_SEMANTICS_PERCENTAGES=50,50
    export SEQNUM_READ_PERCENTAGES=25,25,25,0,25
...
elif [[ $WORKLOAD == mix-with-registration ]]; then
    export APPEND_TIMES=1
    export READ_TIMES=1
    export OPERATION_SEMANTICS_PERCENTAGES=50,50
    export SEQNUM_READ_PERCENTAGES=25,25,25,0,25
    ...
    export SHARED_TAGS_CAPACITY=20
    export CONCURRENCY=4
    export REGISTRATION_EVALUATION=true
...
./run_build.sh boki-hybrid $CONTROLLER_SPEC_DIR/boki/eng3-eh1-st4-seq3-ir1-ur1-mr3.json $BASE_DIR/specs/exp-cf$CONCURRENCY.json
...
$HELPER_SCRIPT start-machines --base-dir=$BASE_DIR
./run_build.sh indilog-postpone-caching $CONTROLLER_SPEC_DIR/indilog/eng4-st4-seq3-ix2-agg1-is2-ir1-ur1-mr3-ssmx4.json $BASE_DIR/specs/exp-cf$CONCURRENCY.json

if [[ $REGISTRATION_EVALUATION == 'true' ]];
then
    $HELPER_SCRIPT reboot-machines --base-dir=$MACHINE_SPEC_DIR
    sleep 90
    ...
    cp $MACHINE_SPEC_DIR/indilog/machines_eng4-st4-seq3-ix2-agg1.json $BASE_DIR/machines.json
    $HELPER_SCRIPT start-machines --base-dir=$BASE_DIR
    ./run_build.sh indilog-postpone-registration $CONTROLLER_SPEC_DIR/indilog/eng4-st4-seq3-ix2-agg1-is2-ir1-ur1-mr3-ssmx4.json $BASE_DIR/specs/exp-cf$CONCURRENCY.json
fi
