...
ssh -q $CLIENT_HOST -- docker run -v /tmp:/tmp \
    maxwie/boki-beldibench:sosp-ae \
...
ssh -q $MANAGER_HOST -- TABLE_PREFIX=$TABLE_PREFIX docker stack deploy -c ~/docker-compose-generated.yml -c ~/docker-compose.yml --resolve-image always boki-experiment
sleep 60
